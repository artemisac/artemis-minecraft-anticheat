...
public class Reload extends TimerTask implements CheckReload {
    ...
    public void reloadAssets() {
        ...
        for (Field field : Checks.class.getDeclaredFields()) {
            field.setAccessible(true);
            CheckInformation checkInformation = (CheckInformation) field.get(null);
            checkInformation.init();
        }
        Chat.sendConsoleMessage(Lang.MSG_CONSOLE_CHECKS_RELOAD.replace("%type%", "standard"));
        String permission = ThemeManager.getCurrentTheme().getVerbosePermission();
        Server.v().getOnlinePlayers().parallelStream().forEach(p -> {
            if (p.hasPermission(permission)) {
                p.sendMessage(Chat.translate("&7[&bArtemis&7] &aReloaded checks.yml!"));
            }
        });
    }
    ...
    public List<CheckInfo> getChecks() {
        ...
        for (Field field : Checks.class.getDeclaredFields()) {
            ...
            field.setAccessible(true);
            try {
                CheckInfo checkInformation = (CheckInfo) field.get(null);
