...
@Check(type = Type.AURA, var = "K", threshold = 2)
@ClientVersion
public class AuraBadPacketK extends ArtemisCheck implements PacketHandler {

    private boolean sent;

    public AuraBadPacketK(final PlayerData data, final CheckInformation info) {
        super(data, info);
    }
    ...
    public void handle(final GPacket packet) {
        if (packet instanceof GPacketPlayClientUseEntity) {
            ...
            if (dig.getType().equals(PlayerEnums.DigType.START_DESTROY_BLOCK)
                    || dig.getType().equals(PlayerEnums.DigType.ABORT_DESTROY_BLOCK)
