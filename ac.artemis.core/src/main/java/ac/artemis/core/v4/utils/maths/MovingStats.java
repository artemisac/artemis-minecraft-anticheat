...
public class MovingStats implements Serializable {
    ...
    public double getStdDevReverse(double required) {
        double stdDev = Math.sqrt(variance);

        //the standard deviation is higher than the requirement
        if (stdDev > required) {
            //count it and make sure all match
            if (++windowCount > this.elements.length) {
                return stdDev;
            ...
            //the stand deviation is lesser than required, reset the count
            if (windowCount > 0) {
                windowCount = 0;
            }
