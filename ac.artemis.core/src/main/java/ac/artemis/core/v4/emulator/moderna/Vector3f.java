...
public final class Vector3f {
   ...
   public Vector3f(Point vecIn) {
      this((float)vecIn.getX(), (float)vecIn.getY(), (float)vecIn.getZ());
   }
   ...
   public void clamp(float min, float max) {
      this.x = MathHelper.a(this.x, min, max);
      this.y = MathHelper.a(this.y, min, max);
      this.z = MathHelper.a(this.z, min, max);
   }
   ...
   public boolean normalize() {
      ...
      if ((double)f < 1.0E-5D) {
         ...
         float f1 = ModernaMathHelper.fastInvSqrt(f);
         this.x *= f1;
         this.y *= f1;
         this.z *= f1;
         return true;
